...
const isMultiTokenModule = name => name === 'MultiTokenBridgeLimitsManager.sol' || name === 'MultiTokenForwardingRulesManager.sol' || name === 'TokenFactory.sol' || name === 'MultiTokenFeeManager.sol'
...
const flat = async contractPath => {
  ...
  if (isBridgeToken(name)) {
    ...
    module = ''
  } else if (isMultiTokenModule(name)) {

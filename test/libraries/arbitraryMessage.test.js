...
contract('ArbitraryMessage.sol', async () => {
  describe('unpackHeader', () => {
    it('unpack dataType 0x00', async () => {
      ...
      const result = await messageTest.unpackHeader(message)

      // then
      const { messageId, chainIds, sender, executor, gasLimit, dataType, gasPrice, offset } = result
      ...
      expect(offset.toNumber()).to.be.equal(32 + 20 + 20 + 4 + 1 + 1 + 1 + 2 + 2)
    })
    ...
    it('unpack dataType 0x00 with different chain ids', async () => {
      ...
      for (const [msgSrcChainId, msgDstChainId] of chainIds) {
        ...
        expect(dataType).to.be.equal(msgDataType)
        expect(gasPrice.toString()).to.be.equal(msgGasPrice)
        expect(offset.toNumber()).to.be.equal(
          32 + 20 + 20 + 4 + 1 + 1 + 1 + msgSrcChainId.length / 2 + msgDstChainId.length / 2
        )
      }
    })
    ...
    it('unpack dataType 0x01', async () => {
      ...
      expect(dataType).to.be.equal(msgDataType)
      expect(gasPrice.toString()).to.be.equal(msgGasPrice)
      expect(offset.toNumber()).to.be.equal(32 + 20 + 20 + 4 + 1 + 1 + 1 + 32 + 2 + 2)
    })
    ...
    it('unpack dataType 0x02', async () => {
      ...
      expect(dataType).to.be.equal(msgDataType)
      expect(gasPrice.toString()).to.be.equal(msgGasPrice)
      expect(offset.toNumber()).to.be.equal(32 + 20 + 20 + 4 + 1 + 1 + 1 + 1 + 2 + 2)
    })
  })
  describe('unpackPayload', () => {
    it('unpack payload from the message', async () => {
      // given
      const msgId = '0xbdceda9d8c94838aca10c687da1411a07b1390e88239c0638cb9cc264219cc10'
      ...
      // when
      const messageTest = await MessageTest.new()
      const result = await messageTest.unpackPayload(message, 32 + 20 + 20 + 4 + 1 + 1 + 1 + 2 + 2)

      // then
      expect(result).to.be.equal(msgData)
    })
  })
})
